...
public class FormerContainer extends Container {
    private final TileEntity tileEntity;
    private final PlayerEntity playerEntity;
    private final IItemHandler playerInventory;

    public FormerContainer(int windowId, World world, BlockPos pos, PlayerInventory playerInventory,
            PlayerEntity player) {
        super(ModContainers.FORMER_COMTAINERS.get(), windowId);
        ...
        if (tileEntity != null) {
            tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
                addSlot(new SlotItemHandler(h, 0, 80, 9));
                addSlot(new SlotItemHandler(h, 1, 58, 31));
                addSlot(new SlotItemHandler(h, 2, 102, 31));
                addSlot(new SlotItemHandler(h, 3, 80, 53));
                addSlot(new SlotItemHandler(h, 4, 80, 31));
            });
        }
    }
